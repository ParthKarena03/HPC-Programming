...
ALL_NODES = ["gics1", "gics2", "gics3", "gics4"]
...
def run_benchmark(p, t):
    ...
    cmd = [MPIRUN, "-np", str(p), "--hostfile", HOSTFILE, BINARY, INPUT_FILE, str(t)]

    try:
        # stderr=subprocess.STDOUT merges warnings and errors into one stream to clean up
        process = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=run_env
        )
...
def main():
    if not os.path.exists(INPUT_FILE):
        print(
            "Error: {0} not found. Ensure it is in the codes directory.".format(
                INPUT_FILE
            )
        )
        return
    ...
    header = "\n{0:^12} | {1:^10} | {2:^12} | {3:^10}".format(
        "Total Cores", "MPI (P)", "OMP (T)", "Time (s)"
    )
    print(header)
    print("-" * len(header))
        ...
        for p in PROCESSORS_LIST:
            for t in THREADS_PER_PROC_LIST:
                if p * t == total:
                    ...
                    if val is not None:
                        print(
                            "{0:<12} | {1:<10} | {2:<12} | {3:<10.6f}".format(
                                total, p, t, val
                            )
                        )
                        results_log.append(
                            {"total": total, "p": p, "t": t, "time": val}
                        )
    ...
    print("=" * 60)
    print(
        "{0:<15} | {1:<12} | {2:<12} | {3:<10}".format(
            "Total Cores", "Best P", "Best T", "Best Time"
        )
    )
    print("-" * 60)
    ...
    for total in TARGET_TOTAL_THREADS:
        ...
        if configs:
            ...
            print(
                "{0:<15} | {1:<12} | {2:<12} | {3:<10.6f}".format(
                    total, best["p"], best["t"], best["time"]
                )
            )
